...
def feedforward(inp, fc_sizes, keep_prob=1.0):#morenzhi
    prev = inp
    i = 0
    for hsize in fc_sizes:
        print('83 row net keep_prob', keep_prob)
...
def sequence_cnn_model(XDinput, XTinput, embedding_size, name='sequence_cnn'):
    ...
    encode_smiles = tf.reduce_max(encode_smiles, [1], name='pool', keep_dims=False)
    ...
    encode_protein = tf.reduce_max(encode_protein, [1], name='pool', keep_dims=False)
...
def make_SB_network(isize=20, in_chnls=19, osize=1, max_smi_len=100, max_seq_len=1000, 
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('sequence_convolution'):
            sequence_cnn = sequence_cnn_model(XD, XT, 128)
        ...
        with tf.variable_scope('training'):
           ...
           #为什么不直接用global_step=tf.Variable(0, trainable=False) ？？？？
            ...
            with tf.variable_scope('L2_cost'):
                ...
                all_weights = [
                    graph.get_tensor_by_name('structure_convolution/conv%s/w:0' % i)
